...
(defn column-offset [column] (cond
                               ...
                               (= column 2) [0 2.82 -2.5]
                               (= column 3) [0 -5 0]
                               (>= column 4) [0 -23 2.50]            ; original [0 -5.8 5.64]
                               :else [0 -5 1.5]))
...
(def mid-wall-position (key-position 0 0 (map + (wall-locate3  1  1) [0 (- (/ mount-height 2) (/ wall-thickness 2)) 0])))
(def usb-holder-position [(first mid-wall-position) (second mid-wall-position) 0])
(def trrs-holder-hole
  (translate usb-holder-position 
             (union
               (->> (cube 31.75 18 15.25) (translate [0 +10.75 7.625]))
               (->> (cube 29 5 12) (translate [0 0 6])) 
               (->> (cube 33.3 18 12) (translate [0 -10.75 6]))
               )
             )
)
...
(defn screw-insert-all-shapes [bottom-radius top-radius height]
      (union (screw-insert 0 0         bottom-radius top-radius height [5.5 7 0])
             (screw-insert 0 lastrow   bottom-radius top-radius height (if trackball-enabled [-2.5 33 0] [-0.5 15 0]))
             (screw-insert 2 0  bottom-radius top-radius height [-3 -2 0])
             ...
             (screw-insert 1 lastrow         bottom-radius top-radius height [-9 -30 0])
             ))
